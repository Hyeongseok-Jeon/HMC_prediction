...
for epoch in range(config["epoch"]):
    ...
    for i, data in enumerate(dataloader_train):
        ...
        batch_accuracy, batch_loss, batch_num, full_length_num = model(trajectory, traj_length)
        batch_loss.backward()
        optimizer.step()
        lr = optimizer.update_learning_rate()

        correct_num_tot += int(batch_accuracy * full_length_num)
        full_length_num_tot += full_length_num
    ...
    logger.info('===> Train Epoch: {} \t Accuracy: {:.2f}% \t Loss: {:.8f}'.format(
        epoch + 1, 100 * correct_num_tot / full_length_num_tot, -loss_tot / calc_num_tot
    ))
    ...
    if (epoch + 1) % config['validataion_period'] == 0:
        ...
        for i, data in enumerate(dataloader_val):
            ...
            full_length_num_tot_val += full_length_num
        ...
        logger.info('===> Validation after Training epoch: {} \t Accuracy: {:.2f}%\tLoss: {:.8f}'.format(
            epoch + 1, 100 * correct_num_tot_val / full_length_num_tot_val, -loss_tot_val / calc_num_tot_val
        ))
        model.train()
