...
for test in range(len(ckpt_list_seq)):
    ...
    for i, data in enumerate(dataloader_train):
        ...
        if i == 0:
            ...
            context_bag_tot = [None if repres is None else repres.cpu().detach().numpy() for repres in representation_time_bag]
            ...
            for j in range(len(representation_time_bag)):
                if representation_time_bag[j] is None:
                    ...
                    if context_bag_train[j] is None:
                        ...
                        context_bag_tot[j] = representation_time_bag[j].cpu().detach().numpy()
                        maneuver_bag_tot[j] = np.repeat(maneuvers, config["val_augmentation"], axis=0)
                    else:
